...
import _ITEM from "prismarine-item"
const Item = _ITEM("1.16.4")
import pathfinder, { path as _path, walk } from "./pathfinder.js"
import vec3 from "vec3"
import { blocks as _blocks, recipes as _recipes } from "./mc-finder.js"
import { readFileSync } from "fs"

const tools = readFileSync("tool-list.txt", "utf8").split("\r\n")
const smelting = JSON.parse(readFileSync("smelting.json", "utf8"))
...
const pathfind = async (bot, position, range = 1, maxLoops = 300) => {
  ...
  let path = _path(bot, bot.entity.position, position, range, maxLoops)

  if (!path.length) {
    await sleep(2000)
    botPosition = bot.entity.position
    path = _path(bot, bot.entity.position, position, range, maxLoops)
  }
  ...
  while (botPosition.distanceTo(position) > range) {
    ...
    if (path.length) {
      ...
      if (path.length) walk(bot, path[path.length - 1].position)
    }
...
const getItem = async (bot, item) => {
  ...
  let sourceBlocks = _blocks(item).map((block) => {
...
const craftItem = async (bot, item) => {
  ...
  let recipes = _recipes(bot, item, false)
  ...
  if (!recipes.length) {
    ...
    recipes = _recipes(bot, item, true)
  }
...
const deposit = async (bot, position) => {
