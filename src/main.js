...
function createBot() {
  ...
  let updatePath = () => {
    ...
    bot.path.path = actions.pathfinder.path(
      bot,
      start,
      end,
      bot.path.range,
...
function runCommand(bot, tokens) {
  switch (tokens[0]) {
    case "break":
      actions.clearBlock(
        bot,
        vec3(parseInt(tokens[1]), parseInt(tokens[2]), parseInt(tokens[3]))
      )
      break
    case "collect":
    ...
    case "come":
      actions.pathfind(
        bot,
        bot.players["Makkusu_Otaku"].entity.position,
        2.5,
        300
      )
      break
    ...
    case "get":
      ...
      break
    case "give":
      actions.give(bot, tokens[1], tokens[2], parseInt(tokens[3] || "1"))
      break
    case "goto":
...
async function cosmicLooper() {
  ...
  if (
    ...
    time - lastJoin > settings.spawnDelay
  ) {
    createBot()
  }
