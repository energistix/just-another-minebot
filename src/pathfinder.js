...
function neighbours(bot, p) {
  let points = [p.offset(0, 0, -1), p.offset(0, 0, 1), p.offset(-1, 0, 0), p.offset(1, 0, 0)]
...
function gridWalk(bot, goal) {
  ...
  goal = goal.offset(-bot.entity.position.x, -bot.entity.position.y, -bot.entity.position.z)
...
function pathfind(bot, start, end, range = 1, maxLoops = 100) {
  ...
  start = vec3(Math.floor(start.x) + 0.5, Math.floor(start.y), Math.floor(start.z) + 0.5)
...
export const path = pathfind
export const walk = gridWalk
